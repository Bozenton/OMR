 ...
 * @brief   IMU姿态解算
 *          This code is largely based on DJI's code:
 *          https://github.com/RoboMaster/Development-Board-C-Examples/blob/master/20.standard_robot/application/INS_task.c
 * @version 0.1
 * @date 2022-12-06
 *
...
#define IMU_temp_PWM(pwm) imu_pwm_set(pwm) // set pwm for temperature control
...
/**
 * @brief          旋转陀螺仪,加速度计和磁力计,并计算零漂,因为设备有不同安装方式
 *                 由于角速度、线性加速度、磁场方向都是一维向量，所以用矩阵乘法即可
 *                 Gyro = R*GyroRaw + offset
 * @param[out]     gyro: 加上零漂和旋转
 * @param[out]     accel: 加上零漂和旋转
 * @param[out]     mag: 加上零漂和旋转
 * @param[in]      bmi088: 陀螺仪和加速度计数据
...
static void imu_temp_control(fp32 temp)
{
  ...
  if (first_temperate)
  {
    PID_calc(&imu_temp_pid, temp, TEMPERATURE_TARGET);
